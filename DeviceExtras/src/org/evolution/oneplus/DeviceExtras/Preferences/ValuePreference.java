...
public class ValuePreference extends CustomSeekBarPreference {
    ...
    private static final int NODE_LEVEL = R.string.node_value_preference;

    public ValuePreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        mInterval = context.getResources().getInteger(R.integer.kcal_value_preference_interval);
        mShowSign = false;
        mUnits = "";
    ...
    private void saveValue(String newValue) {
        ...
        editor.putString(KcalSettings.KEY_VALUE, newValue);
        editor.apply();
    }
