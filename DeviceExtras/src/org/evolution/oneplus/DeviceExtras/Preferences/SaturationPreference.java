...
public class SaturationPreference extends CustomSeekBarPreference {

    private static int mDefVal;

    private static final int NODE_LEVEL = R.string.node_saturation_preference;

    public SaturationPreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        mInterval = context.getResources().getInteger(R.integer.kcal_saturation_preference_interval);
        mShowSign = false;
        mUnits = "";
        mContinuousUpdates = false;

        int[] mAllValues = context.getResources().getIntArray(R.array.kcal_saturation_preference_array);
    ...
    public static void restore(Context context) {
        ...
        int[] mAllValues = context.getResources().getIntArray(R.array.kcal_saturation_preference_array);
        mDefVal = mAllValues[0];
        String storedValue = PreferenceManager.getDefaultSharedPreferences(context).getString(KcalSettings.KEY_SATURATION, String.valueOf(mDefVal));
    ...
    private void saveValue(String newValue) {
        ...
        editor.putString(KcalSettings.KEY_SATURATION, newValue);
        editor.apply();
    }
